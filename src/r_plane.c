...
void R_DrawWaterSpan (void)
{
    ...
    //UINT8*               brighten = transtables+(84<<8);
    UINT8*               brighten = colormaps;
...
void R_DrawWaterSpan_8 (void)
{
    ...
    //UINT8*               brighten = transtables+(84<<8);
    UINT8*               brighten = colormaps-(8*256);
...
void R_DrawSinglePlane(visplane_t* pl, boolean handlesource)
{
  ...
  if(handlesource)
    ds_source = (UINT8 *) R_GetFlat (levelflats[pl->picnum].lumpnum);
