...
// Lactozilla: Heritage
static inline boolean P_ThingIsEnemy (mobj_t *thing)
{
	switch (thing->type)
	{
...
static inline boolean P_CanDamageEnemyFromAbove (mobj_t *thing)
{
	if (!P_ThingIsEnemy(thing))
		return false;
...
#ifdef HERITAGE_ENEMY_COLLISION
		case MT_GFZFISH: // Jumping Fish
#endif
		case MT_JETTBOMBER: // Jetty-Syn Bomber
		case MT_JETTGUNNER: // Jetty-Syn Gunner
...
static inline void P_DamageEnemy(mobj_t *target, mobj_t *inflictor)
{
	...
	P_DamageMobj(target, inflictor, inflictor, 1);
}
...
boolean PIT_CheckThing (mobj_t* thing)
{
			...
			if((tmthing->player->mfjumped == 1) || (tmthing->player->mfspinning == 1) || (tmthing->player->powers[pw_invulnerability]) || (tmthing->player->powers[pw_super])) // Do you possess the ability to subdue the object?
			{
				switch(thing->type)
				{
					...
					case MT_SPEEDPAD: // Speed Pad
						...
						break;
					case MT_EGGMOBILE: // Boss 1
						// The older indentation implied all momentum axes were inverted.
						if(tmthing->momz < 0)
							tmthing->momz = -tmthing->momz;
						tmthing->momx = -tmthing->momx;
						tmthing->momy = -tmthing->momy;
					...
					default:
#ifdef HERITAGE_ENEMY_COLLISION
						if (P_CanDamageEnemyFromAbove(thing))
						{
							if(tmthing->momz < 0)
								tmthing->momz = -tmthing->momz;
							P_DamageEnemy(thing, tmthing);
						}
#endif
						break;
				}
	...
	{
		fixed_t pz = (thing->z + thing->height + FRACUNIT);

		// Are you touching the bottom of it?
#ifdef HERITAGE_ENEMY_COLLISION
		if (pz >= tmthing->z && (pz < tmthing->z + (thing->height / 2))) // Lactozilla: More accurate.
#else
		if (pz == tmthing->z)
#endif
		{
			...
			{
				if (P_ThingIsEnemy(tmthing))
					P_DamageEnemy(tmthing, thing);

				if (tmthing->type == MT_EGGMOBILE) // Boss 1
				{
					// The older indentation implied all momentum axes were inverted.
					if(tmthing->momz > 0)
						tmthing->momz = -tmthing->momz;
			...
			else if (P_ThingIsEnemy(tmthing))
...
static void PTR_SlideDoClimb (angle_t climbangle)
{
		...
		if(slidemo->player) // Whole hella climbin' stuff goin' on here, sucka! Tails 04-12-2001
		{
			if(slidemo->player->gliding || slidemo->player->climbing)
			{
...
#endif
				{
					...
					{
						if ((!slidemo->player->climbing &&
							(((linequadrant == 1) && (playerquadrant == 2))
						...
						{
							PTR_SlideDoClimb(R_PointToAngle2(li->v1->x, li->v1->y, li->v2->x, li->v2->y) + ANG90);
						}
					}
