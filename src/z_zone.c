...
static void Command_Memfree_f(void)
{
	...
	CONS_Printf("Static (Dave)          : %7s KB\n"), sizeu1(Z_TagUsage(PU_DAVE)>>10);
	CONS_Printf("Locked cache           : %7s KB\n"), sizeu1(Z_TagUsage(PU_CACHE)>>10);
	...
	{
		CONS_Printf("Patch info headers     : %7s KB\n"), sizeu1(Z_TagUsage(PU_3DFXPATCHINFO)>>10);
		CONS_Printf("Cached textures        : %7s KB\n"), sizeu1(Z_TagUsage(PU_3DFXCACHE)>>10);
		CONS_Printf("Texture colormaps      : %7s KB\n"), sizeu1(Z_TagUsage(PU_3DFXPATCHCOLMIPMAP)>>10);
	}
#endif

	CONS_Printf("\x82%s", "System Memory Info\n");
