...
FUNCNORETURN static void signal_handler(INT32 num)
{
...
FUNCNORETURN static void quit_handler(int num)
{
	signal(num, SIG_DFL); //default signal action
	raise(num);
	I_Quit();
...
static int joy_open(int joyindex)
{
	...
	if (JoyInfo.dev)
	{
		...
		CONS_Printf("Joystick1 device is changing; resetting events...\n");
		I_ShutdownJoystick();
	}
	...
	if (JoyInfo.dev == NULL)
	{
		CONS_Printf("Joystick1: Couldn't open device - %s\n", SDL_GetError());
		return -1;
	}
	else
	{
		CONS_Printf("Joystick1: %s\n", SDL_JoystickName(JoyInfo.dev));
...
void I_InitJoystick(void)
{
	...
	if (newjoy && joy_open(cv_usejoystick.value) != -1)
	{
...
static void I_PoolMouse2(void)
{
	...
	{
		CONS_Printf("%s", "Read Error on secondary mouse port\n");
		return;
	}
...
void I_StartupMouse2(void)
{
	...
	if (mouse2filehandle == INVALID_HANDLE_VALUE)
	{
		...
		if (mouse2filehandle == INVALID_HANDLE_VALUE)
		{
			...
			if (e == 5)
				CONS_Printf("Can't open %s: Access denied\n", cv_mouse2port.string);
			else
				CONS_Printf("Can't open %s: error %d\n", cv_mouse2port.string, e);
			return;
		}
	}
...
void I_StartupTimer(void)
{
	...
	tic_frequency   = timer_frequency / (double)TICRATE;
}
...
void I_Quit(void)
{
	...
	if (!M_CheckParm("-noendtxt") && W_CheckNumForName("ENDOOM") >= 0)
	{
		printf("\r");
		ShowEndTxt();
	}
...
static long get_entry(const char* name, const char* buf)
{
	...
	if (errno != 0) {
		CONS_Printf("get_entry: strtol() failed: %s\n", strerror(errno));
		return -1;
	}
	return val;
}
