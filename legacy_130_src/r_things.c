							 ...
							 boolean       flipped )
{
	...
	if (rotation == 0)
	{
		...
		for (r=0 ; r<8 ; r++)
		{
			...
			sprtemp[frame].flip[r] = (UINT8)flipped;
		}
		return;
	}
	...
	sprtemp[frame].flip[rotation] = (UINT8)flipped;
}
...
void R_DrawMaskedColumn (column_t* column)
{
	...
	for ( ; column->topdelta != 0xff ; )
	{
		...
		bottomscreen = sprbotscreen == INT32_MAX ? topscreen + spryscale*column->length : sprbotscreen + spryscale*column->length;

		dc_yl = (topscreen+FRACUNIT-1)>>FRACBITS;
		dc_yh = (bottomscreen-1)>>FRACBITS;

#ifdef R_FAKEFLOORS
		if(windowtop != INT32_MAX && windowbottom != MAXINT)
		{
		...
		if (dc_yl <= dc_yh && dc_yl < vid.height && dc_yh > 0)
		{
			...
			// dc_source = (UINT8 *)column + 3 - column->topdelta;
