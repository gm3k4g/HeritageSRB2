...
void T_MoveFloor(floormove_t* floor)
{
    ...
    //SOM: Don't make noise if speed is INT32_MAX
    if (!(leveltime&7) && floor->speed < INT32_MAX)
    ...
    if (res == pastdest)
    {
        ...
        //SOM: Only make stoping noise if speed is less than INT32_MAX
        if(floor->speed < INT32_MAX)
          S_StartSound((mobj_t *)&floor->sector->soundorg,
                       sfx_pstop);
    }
