...
static UINT8         playernode[MAXPLAYERS];
static UINT32        cl_maketic[MAXNETNODES];
...
static UINT8         playerpernode[MAXNETNODES]; // used specialy for scplitscreen
	   boolean       nodeingame[MAXNETNODES];  // set false as nodes leave game
static UINT32        nettics[MAXNETNODES];     // what tic the client have received
static UINT32        supposedtics[MAXNETNODES];// nettics prevision for smaller packet
static UINT8         nodewaiting[MAXNETNODES];
static UINT32        firstticstosend;          // min of the nettics
static short         consistancy[BACKUPTICS];
static UINT32        tictoclear=0;             // optimize d_cleartic
...
// splitscreen player
void SendNetXCmd2(UINT8 id,void *param,int nparam)
{
	if(demoplayback)
	   return;
...
// endif
boolean AddLmpExtradata(UINT8 **demo_point,int playernum)
{
	int  tic;
...
void ReadLmpExtraData(UINT8 **demo_pointer,int playernum)
{
	...
	{
		int    size=0;
		char   *p=(char *)(*demo_pointer+1); // skip nextra
...
static void SV_SendServerInfo(int node, UINT32 time)
{
	UINT8 *p;
...
static void CL_ConnectToServer(void)
{
	...
	do {
		switch(cl_mode) {
			...
			case cl_askjoin :
				CL_LoadServerFiles();
				CONS_Printf("Opening a portal into another dimension...\n");
#ifdef JOININGAME
	...
	if (I_NetOpenSocket)
	{
		...
		else if (I_NetMakeNodewPort)
		{
			if (COM_Argc() >= 3) // address AND port
				...
				servernode = Net_MakeNode(COM_Argv(1));
		}
		else
		{
...
void Got_KickCmd(char **p,int playernum)
{
	...
	if( msg!=KICK_MSG_PLAYER_QUIT )
		CONS_Printf("%s ",player_names[pnum]);
...
{
	UINT32 timout=I_GetTime()+5*TICRATE;
	UINT32 tictac=I_GetTime();
	...
	if(servernode>0 && servernode<MAXNETNODES && nodeingame[(UINT8)servernode]!=0)
	{
...
void Got_AddPlayer(char **p,int playernum)
{
	...
	static UINT32 sendconfigtic=0xffffffff;
		...
		if(!nodeingame[node])
		{
			...
			switch(netbuffer->packettype) {
				case ASKINFO:
					...
					break;
				case SERVERINFO : // response of client ask request
					...
					break;
				case SERVERREFUSE : // negative response of client join request
					if( cl_mode==cl_waitjoinresponse )
						I_Error("Server refused connection\n\nReason :\n%s" // Tails 04-02-2001
							   ,netbuffer->u.serverrefuse.reason);
					break;
				case SERVERCFG :    // positive response of client join request
				{
					int j;
...
// -------------------------------------------- SERVER RECEIVE ----------
			...
			case NODEKEEPALIVEMIS :
...
#else
					CONS_Printf("player %d kicked [%d] consistancy failure\n",netconsole,realstart);
#endif
				}
				...
				{
					memcpy(&netcmds[maketic%BACKUPTICS][(UINT8)nodetoplayer2[node]]
			...
			case TEXTCMD :
				if( netconsole<0 || netconsole>=MAXPLAYERS )
					UnAcknowledgePacket(node);
				else
				{
					int j;
					UINT32 tic=maketic;

					j=software_MAXPACKETLENGTH-(netbuffer->u.textcmd[0]+2+BASEPACKETSIZE+doomcom->numplayers*sizeof(ticcmd_t));
			...
			case CLIENTQUIT:
				...
				if(netconsole!=-1 && playeringame[netconsole])
				{
					...
					SendNetXCmd(XD_KICK,&buf,2);
					nodetoplayer[node]=-1;
					if(nodetoplayer2[node]!=-1 && nodetoplayer2[node]>=0 && playeringame[(UINT8)nodetoplayer2[node]])
					{
...
// -------------------------------------------- CLIENT RECEIVE ----------
			...
			case SERVERTICS :
				...
				txtpak=(UINT8 *)&netbuffer->u.serverpak.cmds[netbuffer->u.serverpak.numplayers*netbuffer->u.serverpak.numtics];
...
{
	UINT32 realfirsttic,lasttictosend=maketic,i;
	int  j,packsize;
...
void SV_SpawnPlayer(int playernum,void *mo)
{
...
void TryRunTics (int realtics)
{
	...
	if (debugfile && (realtics || neededtic>gametic))
	{
		...
		fprintf (debugfile,
				 "------------ Tryruntic : REAL:%i NEED:%d GAME:%d LOAD: %i\n",
