...
static consvar_t cv_alwaysgrabmouse = {"alwaysgrabmouse","Off",CV_SAVE,CV_OnOff};
...
consvar_t cv_stretch = {"stretch","On",CV_SAVE|CV_CALL,CV_OnOff,Stretch_OnChange};
...
static void SurfaceInfo(const SDL_Surface *infoSurface, const char *SurfaceText)
{
	...
	if (infoSurface->flags&SDL_PREALLOC)
		CONS_Printf("%s", " Uses preallocated memory\n");
	else
		CONS_Printf("%s", " Stored in system memory\n");
	if (infoSurface->flags&SDL_RLEACCEL)
		CONS_Printf("%s", " Colorkey RLE acceleration blit\n");
}
	...
	while (SDL_PollEvent(&evt))
	{
		switch (evt.type)
		{
			...
			case SDL_JOYDEVICEADDED:
				{
					...
					CONS_Printf("Joystick device index %d added\n", evt.jdevice.which + 1);
					...
					if (newjoy && JoyInfo.dev != newjoy) // don't override a currently active device
					{
						if (JoyInfo.dev)
					...
					CONS_Printf("Joystick1 device index: %d\n", JoyInfo.oldjoy);

					// update the menu
					/*if (currentMenu == &OP_JoystickSetDef)
						M_SetupJoystickMenu(0);*/

					if (JoyInfo.dev != newjoy)
			...
			case SDL_JOYDEVICEREMOVED:
				...
				{
					CONS_Printf("Joystick1 removed, device index: %d\n", JoyInfo.oldjoy);
					I_ShutdownJoystick();
				}
...
static SDL_bool Impl_CreateContext(void)
{
	...
	if (rendermode == render_soft)
	{
		...
		if (renderer == NULL)
		{
			CONS_Printf("Couldn't create rendering context: %s\n", SDL_GetError());
			return SDL_FALSE;
		}
...
static SDL_bool Impl_CreateWindow(SDL_bool fullscreen)
{
	...
	if (window == NULL)
	{
		CONS_Printf("Couldn't create window: %s\n", SDL_GetError());
		return SDL_FALSE;
	}
...
static void Impl_VideoSetupSDLBuffer(void)
{
	...
	{
		I_Error("%s", "No system memory for SDL buffer surface\n");
	}
}
...
static void Impl_VideoSetupBuffer(void)
{
	...
	if (!vid.buffer)
	{
		I_Error("%s", "Not enough memory for video buffer\n");
	}
}

#ifdef HWRENDER
static void Impl_StartupOpenGL(void)
{
