...
static void Midiplayer_Onchange(void)
{
	...
	if (Mix_GetMidiPlayer() != cv_midiplayer.value)
	{
		if (Mix_SetMidiPlayer(cv_midiplayer.value)) // <> 0 means error
			CONS_Printf("Midi player error: %s", Mix_GetError());
		else
			restart = true;
	}
	...
	if (stricmp(Mix_GetSoundFonts(), cv_midisoundfontpath.string))
	{
		if (!Mix_SetSoundFonts(cv_midisoundfontpath.string)) // == 0 means error
			CONS_Printf("Sound font error: %s", Mix_GetError());
		else
			restart = true;
	}
...
consvar_t cv_midiplayer = {"midiplayer", "Native", CV_CALL|CV_NOINIT|CV_SAVE, midiplayer_cons_t, Midiplayer_Onchange};
consvar_t cv_midisoundfontpath = {"midisoundfont", "sf2/8bitsf.SF2", CV_CALL|CV_NOINIT|CV_SAVE, NULL, MidiSoundfontPath_Onchange};
consvar_t cv_miditimiditypath = {"midisoundbank", "./timidity", CV_SAVE};
#endif
...
void I_StartupSound(void)
{
	...
	{
		CONS_Printf("Error initializing SDL Audio: %s\n", SDL_GetError());
	...
	{
		CONS_Printf("Error starting SDL_Mixer: %s\n", Mix_GetError());
		// call to start audio failed -- we do not have it
		return;
	}
...
static Mix_Chunk *ds2chunk(void *stream)
{
	...
	// lump header
	ver = READUSHORT(stream); // sound version format?
	if (ver != 3) // It should be 3 if it's a doomsound...
		return NULL; // onos! it's not a doomsound!
	freq = READUSHORT(stream);
	samples = READULONG(stream);
		...
		&& ((UINT8 *)lump)[1] == 0x8B)
	{
		...
		if (zErr == Z_OK) // We're good to go
		{
			...
			if (zErr == Z_STREAM_END) {
				...
				CONS_Printf("Encountered %s when running inflate: %s\n", get_zlib_error(zErr), stream.msg);
			(void)inflateEnd(&stream);
		}
		else // Hold up, zlib's got a problem
			CONS_Printf("Encountered %s when running inflateInit: %s\n", get_zlib_error(zErr), stream.msg);
...
UINT32 I_GetSongLength(void)
{
	...
	else
	{
		...
		if (!length)
			CONS_Printf("Getting music length: music is missing LENGTHMS= tag. Needed for seeking.\n");
		return length;
	}
}
...
boolean I_LoadSong(char *data, size_t len)
{
		...
		&& (UINT8)data[1] == 0x8B)
	{
		...
		memset(inflatedData, 0x00, inflatedLen);
		...
		return false;
#else
		CONS_Printf("Cannot decompress VGZ; no zlib support\n");
		return false;
#endif
	}
	...
	*/
	...
	if (result == OPENMPT_PROBE_FILE_HEADER_RESULT_SUCCESS) // We only cared if it succeeded, continue on if not.
	{
		...
		if (!openmpt_mhandle) // Failed to create module handle? Show error and return!
		{
			...
			CONS_Printf("openmpt_module_create_from_memory2: %s\n", mod_err_str);
			return false;
		}
		else
...
boolean I_PlaySong(boolean looping)
{
	...
	if (fpclassify(song_length) == FP_ZERO && (I_SongType() == MU_OGG || I_SongType() == MU_MP3 || I_SongType() == MU_FLAC))
		CONS_Printf("This song is missing a LENGTHMS= tag! Required to make seeking work properly.\n");
#endif
	...
	if(I_SongType() != MU_MOD && I_SongType() != MU_MID && I_SongType() != MU_MID_EX && !Mix_RegisterEffect(MIX_CHANNEL_POST, count_music_bytes, NULL, NULL))
		CONS_Printf("Error registering SDL music position counter: %s\n", Mix_GetError());
...
void I_SetMusicVolume(int volume)
{
	...
	if (cv_midiplayer.value == MIDI_Native && I_SongType() == MU_MID)
		// HACK: Until we stop using native MIDI,
		// disable volume changes
		music_volume = 31;
	else
