...
boolean LoadGL(void)
{
	...
	{
		CONS_Printf("Could not load OpenGL Library: %s\n"
	...
	if (GLULibname)
	{
		...
		{
			CONS_Printf("Could not load GLU Library: %s\n", GLULibname);
	...
	{
		CONS_Printf("Could not load GLU Library\n");
		CONS_Printf("If you know what is the GLU library's name, use -GLUlib\n");
	}
#endif
	return SetupGLfunc();
...
void OglSdlFinishUpdate(boolean waitvbl)
{
	...
	ClipRect(0, 0, realwidth, realheight, 0.9f);
}
