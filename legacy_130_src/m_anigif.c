...
consvar_t cv_gif_optimize = {"gif_optimize", "On", CV_SAVE, CV_OnOff};
consvar_t cv_gif_downscale = {"gif_downscale", "On", CV_SAVE, CV_OnOff};
consvar_t cv_gif_dynamicdelay = {"gif_dynamicdelay", "On", CV_SAVE, gif_dynamicdelay_cons_t};
consvar_t cv_gif_localcolortable = {"gif_localcolortable", "On", CV_SAVE, CV_OnOff};
...
#define ANIGIF_PALSIZE (sizeof(RGBA_t) * 256)

static FILE *gif_out = NULL;
static INT32 gif_frames = 0;
static UINT64 gif_prevframetime = 0;
static UINT32 gif_delayus = 0; // "us" is microseconds
static UINT8 gif_writeover = 0;

static UINT8 *gif_screen = NULL;
static UINT8 *gif_lastscreen = NULL;
static size_t gif_screensize = 0;
static boolean gif_allocscreen = false;
...
//
static RGBA_t *GIF_getpalette(void)
{
...
#endif
	{
		...
		return (gif_colorprofile ? pLocalPalette : pMasterPalette);
	}
}
...
static void GIF_framewrite(void)
{
	...
	// Lactozilla: Compare the last frame's palette with the current frame's palette, and see if it changed.
	if (gif_localcolortable)
	{
		RGBA_t *framepalette = GIF_getpalette();
		if (gif_framepalette)
			palchanged = memcmp(gif_framepalette, framepalette, ANIGIF_PALSIZE);
		gif_framepalette = framepalette;
	}
	...
	if (gif_optimize && gif_frames > 0 && (!palchanged))
	{
		...
		UINT8 *cur_screen = gif_lastscreen;
		...
		{
			UINT8 *linear = HWR_GetFramebuffer();
	...
	else
	{
		...
		{
			UINT8 *linear = HWR_GetFramebuffer();
			GIF_rgbconvert(linear, gif_lastscreen);
			free(linear);
		}
#endif
	...
	// screen regions are handled in GIF_lzw
	{
		...
		{
			float delayf = ceil(100.0f/TICRATE);
		...
		{
			// the original code
			int d1 = (int)((100.0f/TICRATE)*(gif_frames+1));
			int d2 = (int)((100.0f/TICRATE)*(gif_frames));
			delay = d1-d2;
		}
...
#else
	gif_colorprofile = false;
#endif

	// Lactozilla: Heritage changes
	gif_headerpalette = Z_Malloc(ANIGIF_PALSIZE, PU_STATIC, NULL);
	memcpy(gif_headerpalette, GIF_getpalette(), ANIGIF_PALSIZE);

	gif_allocscreen = (rendermode == render_opengl);
	gif_screensize = 0;
...
void GIF_frame(void)
{
	if (gif_allocscreen)
	{
		size_t size = (vid.width * vid.height);
		if (size != gif_screensize)
		{
			gif_screensize = size;
			gif_screen = Z_Realloc(gif_screen, gif_screensize, PU_STATIC, NULL);
			gif_lastscreen = Z_Realloc(gif_lastscreen, gif_screensize, PU_STATIC, NULL);
		}
	}
	else
	{
		gif_screen = screens[4];
		gif_lastscreen = screens[0];
	}
