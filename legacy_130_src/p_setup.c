...
{
  UINT8 *data = W_CacheLumpNum(lump,PU_STATIC);
  int  i;
  int  num;
  int  mapnum;
		...
		case 272:                       //SoM: 4/4/2000: Just colormap transfer
		  ...
		  else
		  {
			...
			if((num = R_CheckTextureNumForName(msd->bottomtexture)) == -1)
			  ...
			  sd->bottomtexture = num;
			break; // Lactozilla: There was a missing break here
		  }
  #endif
...
void P_SetupLevelSky (void)
{
	...
	if (gamemap != TITLEMAP)
	{
		snprintf (skytexname, sizeof(skytexname), "SKY%d", gamemap);
					  ...
					  char*         wadname)      // for wad files
{
	...
	totalkills = totalitems = totalsecret = totalrings = numplayers = 0; // Tails 08-11-2001
...
#ifdef CHASECAM_SETUPLEVEL_NOCLEAR
	if (camera.chase)
#endif
		camera.mo = NULL;
...
boolean P_AddWadFile (char* wadfilename,char **firstmapname)
{
	...
	if (!devparm && replaces)
		CONS_Printf ("%d sounds replaced\n", replaces); // Lactozilla: This call had no argument for some reason
