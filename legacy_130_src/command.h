...
//-----------------------------------------------------------------------------
//
// $Id: command.h,v 1.3 2000/04/16 18:38:06 bpereira Exp $
//
...
const char *COM_CompleteCommand (char *partial, int skips);
...
typedef struct consvar_s
{
	...
	char *zstring;                     // Either NULL or same as string. If non-NULL, must be Z_Free'd later.
    unsigned short netid;              // used internaly : netid for send end receive, used only with CV_NETVAR
...
// send modified cvar, for join in game
