...
UINT8           consoleplayer;          // player taking events and displaying
UINT8           displayplayer;          // view being displayed
UINT8           secondarydisplayplayer; // for splitscreen
...
UINT8 nextweaponorder[NUMWEAPONS]={wp_fist,wp_chainsaw,wp_pistol,
	 wp_shotgun,wp_supershotgun,wp_chaingun,wp_missile,wp_plasma,wp_bfg};

UINT8 NextWeapon(player_t *player,int step)
{
	byte   w;
	int    i;
...
*/
	...
	cmd->aiming = G_ClipAimingPitch ((INT32 *)&localaiming);
...
*/
	...
	cmd->aiming = G_ClipAimingPitch ((INT32 *)&localaiming2);
...
boolean G_Responder (event_t* ev)
{
		...
		(demoplayback || gamestate == GS_DEMOSCREEN) )
	{
		...
		&& ev->data1 != KEY_CTRL && ev->data1 != KEY_ALT && ev->data1 != KEY_SHIFT) // Lactozilla
		{
			M_StartControlPanel ();
			return true;
		}
...
void G_Ticker (void)
{
	...
	for (i=0 ; i<MAXPLAYERS ; i++)
	{
		...
		if (playeringame[i] || i==0)
		{
			...
			if (demoplayback)
			{
				if (!window_notinfocus) // Lactozilla: Window focus
...
void G_InitNew (skill_t skill, char* mapname, boolean resetplayer)
{
	...
	{
		// external map file
		strlcpy (gamemapname, mapname, MAX_WADPATH);
		gameepisode = 1;
		gamemap = 1;
	}
	else
	{
		...
		{
			CONS_Printf("Internal game map '%s' not found\n"
...
//
	...
	if ((i=W_CheckNumForName(defdemoname)) == -1)
	{
		...
		{
			CONS_Printf ("ERROR: couldn't open file '%s'.\n", defdemoname);
			goto no_demo;
		}
		demo_p = demobuffer;
	}
...
//
	...
	{
		CONS_Printf ("ERROR: demo version too old.\n");
		Z_Free (demobuffer);
no_demo:
		gameaction = ga_nothing;
		return;
	...
	if (demoversion < VERSION)
		CONS_Printf ("Demo is from an older game version\n");
