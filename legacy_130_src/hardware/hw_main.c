					  ...
					  FBITFIELD         PolyFlags )
{
	...
	else
	{
		...
		// Lactozilla: Why was it setting the alpha to the light level?
...
//        v3d->tow =  1; //gr_windowcentery + v3d->y*gr_centery/v3d->oow;
	...
	HWD.pfnDrawPolygon( NULL, planeVerts, nrPlaneVerts, PF_Invisible|PF_Occlude|PF_Clip );
}
#endif //polysky
					  ...
					  FSurfaceInfo * pSurf )
{
	...
	HWD.pfnDrawPolygon( pSurf, trVerts, 4, PF_Modulated|PF_Masked|PF_Occlude|PF_Clip);
/*
...
#endif
{
	...
	fixed_t     worldtop = 0;
	fixed_t     worldbottom = 0;
	fixed_t     worldhigh = 0;
	fixed_t     worldlow = 0;

	fixed_t     worldup = 0;    // for 2sided lines
	fixed_t     worlddown = 0;
	...
	fixed_t     texturehpeg = 0;
	fixed_t     texturevpeg = 0;        //mid or top
	fixed_t     texturevpegtop = 0;     //top
...
#ifdef TANDL
	wallVerts[0].oow = wallVerts[3].oow = spr->z1;
	wallVerts[1].oow = wallVerts[2].oow = spr->z2;
#else
	// make a wall polygon (with 2 triangles), using the floor/ceiling heights,
	// and the 2d map coords of start/end vertices
...
// Lactozilla note: This was fixed to work with T&L.
static void HWR_ProjectSprite (mobj_t* thing)
{
	gr_vissprite_t*     vis;
	...
	if (flip)
	{
		x1 = FIXED_TO_FLOAT(spritewidth[lump] - spriteoffset[lump]);
		x2 = FIXED_TO_FLOAT(spriteoffset[lump]);
	}
	else
	{
		x1 = FIXED_TO_FLOAT(spriteoffset[lump]);
		x2 = FIXED_TO_FLOAT(spritewidth[lump] - spriteoffset[lump]);
	}
  ...
  tr_x = x1;
	...
	if (thing->flags & MF_TRANSLATION)
		vis->colormap = (UINT8 *) translationtables - 256 + ( (thing->flags & MF_TRANSLATION) >> (MF_TRANSSHIFT-8) );
	else
		vis->colormap = colormaps;

	// set top/bottom coords
#ifdef TANDL
	vis->ty = ((float)thing->z * crapmul) + ((float)spritetopoffset[lump] * crapmul );
#else
...
void HWR_RenderPlayerView (int viewnumber, player_t* player)
{
	...
	transform.scalez = 1;
	transform.fovxangle = (float)cv_grfov.value; // Tails
	transform.fovyangle = (float)cv_grfov.value; // Tails
	transform.splitscreen = cv_splitscreen.value;
#endif
...
{
	HWD.pfnSetSpecialState( HWD_SET_FOG_COLOR, atohex(cv_grfogcolor.zstring) );
	HWD.pfnSetSpecialState( HWD_SET_FOG_DENSITY, cv_grfogdensity.value );
