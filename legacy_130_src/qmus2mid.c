...
//-----------------------------------------------------------------------------
//
// $Id: qmus2mid.c,v 1.2 2000/02/26 00:28:42 hurdler Exp $
//
...
int fwritemem(void *p,int size,int number, UINT8 **file)
{
...
#if 0
int fwrite2(USHORT *ptr, int size, UINT8 **file)
{
    UINT32 rev = 0;
    unsigned long i;
...
#else
int fwrite2(UINT8 *ptr, int size, UINT8 **file)
{
    char    rev[4];
...
void TWriteByte (UINT8 MIDItrack, char bbyte)
{
    UINT32 pos;
...
void TWriteVarLen (UINT8 tracknum, UINT32 value)
{
    UINT32 buffer;
    ...
    while( 1 )
    {
        TWriteByte( tracknum, (UINT8)buffer);
        if (buffer & 0x80 )
            buffer >>= 8;
        else
            break;
...
int WriteMIDheader( USHORT ntrks, USHORT division, UINT8 **file )
{
...
void WriteTrack (int tracknum, UINT8 **file)
{
    USHORT size;
    size_t quot, rem;
    ...
    fwritemem (((UINT8 *) track[tracknum].data)+4096*quot, rem, 1, file );
    fwritemem (TRACKMAGIC2, 4, 1, file );
}


void WriteFirstTrack (UINT8 **file)
{
    USHORT size;
...
int qmus2mid (byte  *mus, UINT8 *mid,     // buffers in memory
...
{
    UINT8*    file_mus;
    UINT8*    file_mid;     // pointer in memory
            ...
            (event != EOF) )
    {
        if (MUS2MIDchannel[MUSchannel] == -1)
        {
            ...
            MIDItrack = MIDIchan2track[MIDIchannel] = (UINT8)TrackCnt++;
        ...
        {
          case 0 :                /* release note */
            MidiEvent((UINT8)0x90 | MIDIchannel);

            data = *(file_mus++);
            TWriteByte( MIDItrack, (UINT8)(data & 0x7F));
          ...
          case 2 :
            MidiEvent((UINT8)0xE0 | MIDIchannel);
            ...
            break;
          case 3 :
            MidiEvent((UINT8)0xB0 | MIDIchannel);
          ...
          case 4 :
            ...
            if (data )
            {
                ...
                MidiEvent((UINT8)0xC0 | MIDIchannel);
    ...
    for (i = 0; i < 16; i++)
    {
      ...
      {
            TWriteByte(i, (UINT8)0x00); // midi end of track code
            TWriteByte(i, (UINT8)0xFF);
            TWriteByte(i, (UINT8)0x2F);
            TWriteByte(i, (UINT8)0x00);
