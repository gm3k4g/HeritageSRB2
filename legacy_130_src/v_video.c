...
static CV_PossibleValue_t ticrate_cons_t[] = {{0, "Off"}, {1, "Full"}, {2, "Compact"}, {3, "Graph"}, {0, NULL}};
consvar_t cv_ticrate={"vid_ticrate","Off",0,ticrate_cons_t,NULL};

// local copy of the palette for V_GetColor()
RGBA_t      pMasterPalette[256];
RGBA_t      pLocalPalette[256];
...
{
	return pLocalPalette[color&0xFF].rgba;
}
...
{
	int     i;
	UINT8*   usegamma = gammatable[scr_gamma%5];
	UINT8*   pal = palette;

	// keep a copy of the palette, for V_GetColor()
	for( i=0; i<256; i++ )
	{
		// Lactozilla: pMasterPalette implementation
		UINT8 r = *pal++;
		UINT8 g = *pal++;
		UINT8 b = *pal++;

		pMasterPalette[i].s.red = r;
		pMasterPalette[i].s.green = g;
		pMasterPalette[i].s.blue = b;

		pLocalPalette[i].s.red = usegamma[r];
		pLocalPalette[i].s.green = usegamma[g];
		pLocalPalette[i].s.blue = usegamma[b];
	}
...
// --------------------------------------------------------------------------
void VID_BlitLinearScreen (UINT8* srcptr, UINT8* destptr,
  ...
  patch_t*      patch )
{
	...
	for ( ; col<w ; x++, col++, desttop++)
	{
		column = (column_t *)((UINT8 *)patch + LONG(patch->columnofs[w-1-col]));
...
void V_DrawFadeScreen (void)
{
	...
	unsigned    quad;
	UINT8       p1, p2, p3, p4;
	UINT8*       fadetable = (UINT8 *) colormaps + 16*256;
	//short*    wput;
...
// Lactozilla: Heritage
static void V_DrawMappedString (int x, int y, const char* string, UINT8 *colormap)
{
	int         w;
	const char* ch;
	int         c;
	...
	while(1)
	{
		...
		if (cx+w > vid.width)
			break;
		V_DrawMappedPatch(cx, cy, 0, hu_font[c], colormap);
		cx+=w;
	}
}
...
{
	V_DrawMappedString (x, y, string, whitemap);
}
...
// Lactozilla: Replacement for R_DrawSpanNoWrap
static void R_DrawTiltViewSpan (void)
{
	fixed_t xposition;
	fixed_t yposition;
	fixed_t xstep, ystep;
	...
	{
		*dest++ = source[(((yposition >> FRACBITS) * vid.width) + (xposition >> FRACBITS))];
		xposition += xstep;
		yposition += ystep;
	}
}
...
void V_DrawPerspView (UINT8 *viewbuffer, int aiming)
{
	...
	// +16 to -16 fixed
	offs = (AIMINGTODY(aiming) * 0xC000); //((aiming*20)<<16) / 100;
	...
	memset(screens[0], 0x00, vid.rowbytes * vid.height);

	for (y=0; y<vid.height; y++)
	{
		x1 = ((vid.width<<16) - scale)>>17;
		dest = ((UINT8*) screens[0]) + (vid.rowbytes*y) + x1;
