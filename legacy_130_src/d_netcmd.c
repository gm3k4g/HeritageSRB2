...
// protos
// ------
UINT8 P_Random(); // Tails 07-22-2001
...
void Command_StartMovie_f(void);
void Command_StopMovie_f(void);
...
void D_RegisterClientCommands (void)
{
  ...
  //
	...
	//cv_playername.defaultvalue=I_GetUserName(); // Lactozilla: No
	...
	CV_RegisterVar (&cv_heritage_playermovement);
#endif
#ifdef HERITAGE_DIRECTIONCHAR
	CV_RegisterVar (&cv_heritage_directionchar);
#endif
...
void SendNameAndColor(void)
{
	...
	*(p-1) = 0; // finish the string;
	...
	cv_skin.value=R_SkinAvailable(cv_skin.zstring);
...
void SendNameAndColor2(void)
{
	...
	*(p-1) = 0; // finish teh string;
	...
	// check if player has the skin loaded (cv_skin may have
	//  the name of a skin that was available in the previous game)
	cv_skin2.value=R_SkinAvailable(cv_skin2.zstring);
	if (!cv_skin2.value)
