...
void P_ZMovement (mobj_t* mo)
{
	fixed_t     dist;
	fixed_t     delta;
	fixed_t     gravityadd = 0; // Lactozilla: Moved here because the playergravity jump would not properly initialize it
...
extern UINT8 weapontobutton[NUMWEAPONS];
...
// Lactozilla: Heritage
static inline void P_SpawnStackOfRings(fixed_t x, fixed_t y, fixed_t z, fixed_t dist, subsector_t *ss, mapthing_t *mthing)
{
	...
	for (r = 1; r < 6; r++)
	{
		z = ss->sector->floorheight + dist*r;
...
#if 0 // Lactozilla: This comparison always evaluates to false.
	if (i==NUMMOBJTYPES && (!i == 84 || !i == 44)) // Tails 08-05-2001
	{
		CONS_Printf ("P_SpawnMapThing: Unknown type %i at (%i, %i)\n",
					  mthing->type,
					  mthing->x, mthing->y);
		return;
	}
...
//             || (mobjinfo[i].flags & MF_COUNTKILL)) )
	...
	if (mthing->type == 84) // Vertical Rings - Stack of 5 Tails 08-05-2001
		P_SpawnStackOfRings(x, y, z, 64*FRACUNIT, ss, mthing);
	else if (mthing->type == 44) // Vertical Rings - Stack of 5 (suitable for Red Spring) Tails 08-05-2001
		P_SpawnStackOfRings(x, y, z, 128*FRACUNIT, ss, mthing);
	else if (i != NUMMOBJTYPES)
	{
