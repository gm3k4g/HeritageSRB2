...
#include "m_fixed.h" // Lactozilla: FixedHypot
...
void P_MovePlayer (player_t* player)
{
	...
	fixed_t oldMagnitude = 0, newMagnitude;
	vector3_t totalthrust;
	const INT32 basethrustfactor = 5;
	const INT32 baseaccelstart = 96;
	const INT32 baseacceleration = 40;
	boolean spin;
#endif
...
// CMOMx stands for the conveyor belt speed.
	...
	// Lactozilla: This doesn't compile.
	...
	spin = (onground && player->mfspinning && (player->rmomx || player->rmomy) && !player->mfstartdash);
	...
	if (Heritage_PlayerMovement(player))
	{
		...
		{
			topspeed = 5 * normalspeed / 3; // 1.67x
			thrustfactor = basethrustfactor*2;
			acceleration = baseaccelstart/2 + player->speed * baseacceleration/2;
		}
		else // if not, then run normally Tails 02-28-2000
		{
			topspeed = normalspeed;
			thrustfactor = basethrustfactor;
			acceleration = baseaccelstart + player->speed * baseacceleration;
		}
		...
		if (player->climbing)
		{
			...
			|| ((player->mo->state == &states[S_PLAY_PAIN] && player->powers[pw_invisibility]) && !onground)))
		{
		...
		else if (cmd->sidemove && !(player->gliding) && !player->exiting && !(player->mo->state == &states[S_PLAY_PAIN] && player->powers[pw_invisibility]))
		{
			...
			if (!onground)
			{
				...
				if (player->mfspinning || (player->powers[pw_tailsfly] && player->speed*FRACUNIT > topspeed))
					movepushside >>= 2;
			}
			// Allow a bit of movement while spinning
...
void P_DeathThink (player_t* player)
{
	...
	if (attacker && attacker != player->mo)
	{
		...
		player->aiming = G_ClipAimingPitch ((INT32 *)&pitch);
...
// Lactozilla note: There is a weird issue whenever the player respawns without
// the chasecam on. Most of the times it's a crash; sometimes, the player spawns
// stuck in somewhere. Not sure what's up and if it's undesirable enough to fix

// addendum: I later figured it out. Define CHASECAM_SETUPLEVEL_NOCLEAR
...
static void P_DeezNux (player_t *player)
{
	...
	else if (player->mo->state == &states[S_PLAY_PAIN] && player->powers[pw_invisibility])
		;
	else
	{
		if (player->gliding)
		...
		{
			diff = ((player->mo->angle + ((cv_analog.value) ? 0 : R_PointToAngle2(0, 0, cmd->forwardmove<<FRACBITS, -cmd->sidemove<<FRACBITS))) - player->drawangle);
			factor = 4;
		}
	...
	if (!player->running && !player->walking && !player->spinning)
	{
		...
		if (diff > ANG10/2)
		{
			...
			if (stat >= S_PLAY_TAP1 && stat <= S_PLAY_TAP2)
				P_SetMobjState(player->mo, S_PLAY);
			else if (stat == S_PLAY && player->mo->tics != -1)
				player->mo->tics++;
		}
	}
}
